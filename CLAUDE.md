...
> Repo-specific note: in this workspace, `npx gitnexus analyze` may fail on Windows with `.gitnexus` permission errors such as `Cannot open file ...\\.gitnexus\\lbug - Access is denied` or `EPERM: operation not permitted, mkdir '.gitnexus\\csv'`. Treat that as an index-write environment issue, not as a code issue in the repo.

## Always Do

- **MUST run `npx gitnexus status` before trying to refresh the index.** Confirm whether the index is stale and whether `.gitnexus/` is writable first.
...
- **MUST prefer CLI fallback commands when MCP GitNexus tools are not available in the current Codex session.** Use:
...
- **MUST report stale-index status when working from an old index.** If `status` shows `Indexed commit != Current commit`, say so explicitly in the user-facing update.
- **MUST stop retrying `analyze` after a confirmed permission failure.** One failed refresh with `.gitnexus\\lbug` / `.gitnexus\\csv` access errors is enough; continue with the existing index and record the blockage.
...
- NEVER blindly loop on `npx gitnexus analyze` when the failure is `Access is denied` / `EPERM` inside `.gitnexus/`; repeated retries do not fix the problem.
...
4. If `analyze` fails with `.gitnexus` permission errors, stop retrying and fall back to the stale index.

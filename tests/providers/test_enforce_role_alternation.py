    ...
    messages = [{
        ...
        "content": [
            ...
            },
            {
                ...
                "_meta": {"path": "/tmp/image.png"},
            },
        ],
    }]

    sanitized = LLMProvider.sanitize_for_provider(messages)

    assert "_meta" in messages[0]["content"][0]
    assert "_meta" in messages[0]["content"][1]
    assert sanitized is not messages
    assert sanitized[0] is not messages[0]
    assert sanitized[0]["content"][0] is not messages[0]["content"][0]
    assert "_meta" not in sanitized[0]["content"][0]
    assert "_meta" not in sanitized[0]["content"][1]
...
class TestEnforceRoleAlternation:
    ...
    def test_trailing_assistant_recovered_as_user_when_only_system_remains(self):
        ...
        When a caller produces [system, assistant], _enforce_role_alternation
        would drop the assistant, leaving only [system].

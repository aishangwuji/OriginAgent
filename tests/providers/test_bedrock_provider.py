...
def test_bedrock_provider_is_registered_and_matches_without_api_key() -> None:
    ...
    cfg = Config.model_validate({
        "agents": {"defaults": {"provider": "auto", "model": "bedrock/global.anthropic.claude-opus-4-7"}},
...
def test_build_kwargs_strips_meta_without_mutating_messages() -> None:
    provider = BedrockProvider(region="us-east-1", client=FakeClient())
    messages = [{
        "role": "user",
        "content": [
    ...
    assert kwargs["messages"][0]["content"] == [{"text": "hi"}]

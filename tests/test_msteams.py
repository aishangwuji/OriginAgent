...
if not MSTEAMS_AVAILABLE:
    pytest.skip("MSTeams dependencies not installed (PyJWT, cryptography). Run: pip install OriginAgent[msteams]", allow_module_level=True)

import jwt
from cryptography.hazmat.primitives.asymmetric import rsa

import OriginAgent.channels.msteams as msteams_module
from OriginAgent.bus.events import OutboundMessage
from OriginAgent.channels.msteams import ConversationRef, MSTeamsChannel
...
def test_strip_possible_bot_mention_removes_generic_at_tags(make_channel):
    ...
    assert ch._strip_possible_bot_mention("<at>OriginAgent</at> hello") == "hello"
...
def test_sanitize_inbound_text_keeps_normal_inline_message(make_channel):
    ...
    activity = {
        "text": "<at>OriginAgent</at> normal inline message",
        "channelData": {},
    }
...
async def test_start_logs_install_hint_when_pyjwt_missing(make_channel, monkeypatch):
    ...
    assert errors == ["PyJWT not installed. Run: pip install OriginAgent[msteams]"]

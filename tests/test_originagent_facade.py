"""Tests for the OriginAgent programmatic facade."""
...
def _write_config(tmp_path: Path, overrides: dict | None = None) -> Path:
    data = {
        ...
        "agents": {"defaults": {"provider": "openrouter", "model": "openai/gpt-4.1"}},
    }
    if overrides:
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        OriginAgent.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = OriginAgent.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("OriginAgent.config.loader.load_config") as mock_load, \
         patch("OriginAgent.providers.factory.make_provider") as mock_prov:
        mock_load.return_value = Config()
        mock_prov.return_value = MagicMock()
        mock_prov.return_value.get_default_model.return_value = "test"
        mock_prov.return_value.generation.max_tokens = 4096
        OriginAgent.from_config()
...
def test_workspace_override(tmp_path):
    ...
    bot = OriginAgent.from_config(config_path, workspace=custom_ws)
...
def test_import_from_top_level():
    import OriginAgent as originagent_module

    assert originagent_module.OriginAgent is OriginAgent
    assert originagent_module.RunResult is RunResult

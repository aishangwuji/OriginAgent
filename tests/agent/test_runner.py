...
async def test_runner_sanitizes_provider_messages_without_mutating_internal_messages():
    ...
    assert result.final_content == "done"
    assert "_meta" in initial_messages[0]["content"][0]
    assert "_meta" in result.messages[0]["content"][0]
    assert captured_messages[0]["content"][0] is not initial_messages[0]["content"][0]
    assert "_meta" not in captured_messages[0]["content"][0]
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    trimmed = runner._snip_history(spec, provider, messages)
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    ...
    assert "second question" in _content_text(non_system[2]["content"])

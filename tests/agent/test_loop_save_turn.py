...
def test_save_turn_drops_runtime_and_reference_blocks_but_keeps_internal_event() -> None:
    loop = _mk_loop()
    session = Session(key="test:context-kinds")
        ...
        [{
            ...
            "content": [
                ...
                },
                {
                    "type": "text",
                    "text": "reference",
                    "_meta": {"kind": ContextBuilder.REFERENCE_CONTEXT_KIND},
                },
                {
                    "type": "text",
                    "text": "internal event",
                    "_meta": {"kind": ContextBuilder.INTERNAL_EVENT_KIND},
                },
...
async def test_stop_preserves_runtime_checkpoint_for_next_turn(tmp_path: Path) -> None:
    ...
    loop._active_tasks[first_msg.session_key] = [(task, time.time())]
...
async def test_system_subagent_followup_is_persisted_before_prompt_assembly(tmp_path: Path) -> None:
    ...
    subagent_context = non_system[2]["content"]
    assert _content_text(subagent_context).count("subagent result") == 1
    assert '"current_time"' in _content_text(subagent_context)
    assert any(
        isinstance(block, dict)
        and (block.get("_meta") or {}).get("kind") == ContextBuilder.INTERNAL_EVENT_KIND
        for block in subagent_context
    )

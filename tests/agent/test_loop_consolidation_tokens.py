...
def _archive_result(summary: str = "summary", cursor: int = 1) -> memory_module.ArchiveResult:
    return memory_module.ArchiveResult(summary=summary, history_cursor=cursor)
...
async def test_prompt_below_threshold_does_not_consolidate(tmp_path) -> None:
    ...
    loop.consolidator.archive = AsyncMock(return_value=_archive_result())  # type: ignore[method-assign]
...
async def test_consolidation_persists_summary_for_next_prepare_session(tmp_path, monkeypatch) -> None:
    ...
    loop.consolidator.archive = AsyncMock(
        return_value=_archive_result("User discussed project status.", 9)
    ...
    meta = reloaded.metadata.get("_recent_summaries")
    assert isinstance(meta, list)
    assert meta[0]["text"] == "User discussed project status."
    assert meta[0]["history_cursor"] == 9

    reloaded, pending = loop.auto_compact.prepare_session(reloaded, "cli:test")
    assert pending is not None
    assert "User discussed project status." in pending
    assert "[history cursor 9]" in pending

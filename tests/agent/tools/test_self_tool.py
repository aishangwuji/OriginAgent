...
class TestModifyBlocked:
    ...
    async def test_modify_dotpath_leaf_dunder_blocked(self):
        ...
        assert "dot-path set is not allowed" in result
...
class TestModifyFree:
    ...
    @pytest.mark.asyncio
    async def test_modify_existing_attr_rejected_unless_restricted(self):
        """Only explicit restricted settings and scratchpad keys can be set."""
        tool = _make_tool()
        result = await tool.execute(action="set", key="provider_retry_mode", value="persistent")
        assert "scratchpad key 'provider_retry_mode' is reserved" in result
    ...
    async def test_modify_existing_attr_type_mismatch_rejected(self):
        """Setting arbitrary existing attrs is rejected."""
        tool = _make_tool()
    ...
    async def test_modify_existing_int_attr_wrong_type_rejected(self):
        """Setting arbitrary existing int attrs is rejected."""
        tool = _make_tool()
...
class TestModifyOpen:
    ...
    @pytest.mark.asyncio
    async def test_modify_workspace_blocked(self):
        """workspace cannot be shadowed or modified through my.set."""
        tool = _make_tool()
        result = await tool.execute(action="set", key="workspace", value="/new/path")
        assert "protected" in result or "reserved" in result
...
class TestRuntimeVarsLimits:
    ...
    @pytest.mark.asyncio
    async def test_runtime_vars_rejects_dot_key(self):
        tool = _make_tool()
        result = await tool.execute(action="set", key="web_config.foo", value="x")
        assert "read-only" in result or "dot-path set is not allowed" in result

    @pytest.mark.asyncio
    async def test_runtime_vars_rejects_reserved_key(self):
        tool = _make_tool()
        result = await tool.execute(action="set", key="exec_config", value="x")
        assert "read-only" in result

    @pytest.mark.asyncio
    async def test_runtime_vars_rejects_sensitive_camel_case_key(self):
        tool = _make_tool()
        result = await tool.execute(action="set", key="accessToken", value="secret")
        assert "protected" in result or "reserved" in result

    @pytest.mark.asyncio
    async def test_inspect_fallback_hides_invalid_runtime_var_keys(self):
        loop = _make_mock_loop()
        loop._runtime_vars = {
            "valid_key": "visible",
            "web_config.foo": "hidden",
            "accessToken": "hidden",
            "workspace": "hidden",
        }
        tool = _make_tool(loop)

        assert "visible" in await tool.execute(action="check", key="valid_key")
        assert "not found" in await tool.execute(action="check", key="web_config.foo")
        assert "not accessible" in await tool.execute(action="check", key="accessToken")
        scratchpad = await tool.execute(action="check", key="scratchpad")
        assert "valid_key" in scratchpad
        assert "accessToken" not in scratchpad
        assert "web_config.foo" not in scratchpad
...
class TestSubagentStatusFormatting:
    ...
    def test_format_single_status(self):
        ...
        from OpenHome.agent.subagent import SubagentStatus
...
class TestSubagentHookStatus:
    ...
    async def test_after_iteration_updates_status(self):
        ...
        from OpenHome.agent.subagent import SubagentStatus, _SubagentHook
    ...
    async def test_after_iteration_no_status_is_noop(self):
        ...
        from OpenHome.agent.subagent import _SubagentHook
        from OpenHome.agent.hook import AgentHookContext
...
class TestRuntimeVarsInspectFallback:
    ...
    async def test_inspect_runtime_var_string(self):
        tool = _make_tool()
        await tool.execute(action="set", key="current_project", value="OpenHome")
        result = await tool.execute(action="check", key="current_project")
        assert "OpenHome" in result

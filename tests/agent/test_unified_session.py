...
from OpenHome.command.builtin import cmd_new, register_builtin_commands
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("OpenHome.agent.loop.SessionManager"), \
         patch("OpenHome.agent.loop.SubagentManager") as MockSubMgr, \
         patch("OpenHome.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates OpenHome onboard)."""
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from OpenHome.agent.memory import Consolidator, MemoryStore

...
from OpenHome.agent.memory import ArchiveResult, session_summary_text
from OpenHome.bus.events import InboundMessage
from OpenHome.bus.queue import MessageBus
from OpenHome.command import CommandContext
...
class TestSessionTTLConfig:
    ...
    def test_session_file_cap_is_internal_constant(self):
        ...
        from OpenHome.session.manager import FILE_MAX_MESSAGES
...
class TestAutoCompact:
    ...
    async def test_check_expired_only_archives_expired_sessions(self, tmp_path):
        ...
        async def _fake_archive(messages):
            return _archive_result("Summary.")
    ...
    async def test_auto_compact_stores_summary(self, tmp_path):
        ...
        async def _fake_archive(messages):
            return _archive_result("User said hello.")
        ...
        assert "User said hello." in entry
        assert "[history cursor 1]" in entry
        session_after = loop.sessions.get_or_create("cli:test")
        assert len(session_after.messages) == loop.auto_compact._RECENT_SUFFIX_MESSAGES
        await loop.close_mcp()
...
class TestProactiveAutoCompact:
    ...
    async def test_proactive_archive_on_idle_tick(self, tmp_path):
        ...
        async def _fake_archive(messages):
            archived_messages.extend(messages)
            return _archive_result("User chatted about old things.")
        ...
        entry = loop.auto_compact._summaries.get("cli:test")
        assert entry is not None
        assert "User chatted about old things." in entry
        assert "[history cursor 1]" in entry
        await loop.close_mcp()
...
class TestSummaryPersistence:
    ...
    async def test_summary_persisted_in_session_metadata(self, tmp_path):
        """After archive, _recent_summaries should be in session metadata."""
        loop = _make_loop(tmp_path, session_ttl_minutes=15)
        session = loop.sessions.get_or_create("cli:test")
        _add_turns(session, 6, prefix="hello")
        session.updated_at = datetime.now() - timedelta(minutes=20)
        ...
        session_after = loop.sessions.get_or_create("cli:test")
        meta = session_after.metadata.get("_recent_summaries")
        assert isinstance(meta, list)
        assert meta[0]["text"] == "User said hello."
        assert meta[0]["history_cursor"] == 1
        assert "created_at" in meta[0]
        assert "last_active" in meta[0]
    ...
    async def test_summary_recovered_after_restart(self, tmp_path):
        ...
        assert "[history cursor 1]" in summary
        # _recent_summaries persists in metadata for restart survival.
        assert "_recent_summaries" in reloaded.metadata
        assert "_last_summary" not in reloaded.metadata
        await loop.close_mcp()

    @pytest.mark.asyncio
    async def test_metadata_persists_for_restart(self, tmp_path):
        """_recent_summaries stays in metadata so it survives process restarts."""
        loop = _make_loop(tmp_path, session_ttl_minutes=15)
        session = loop.sessions.get_or_create("cli:test")
        _add_turns(session, 6, prefix="hello")
        session.updated_at = datetime.now() - timedelta(minutes=20)
    ...
    @pytest.mark.asyncio
    async def test_inmemory_path_matches_recent_summary_metadata(self, tmp_path):
        """In-memory _summaries path should match _recent_summaries metadata format."""
        loop = _make_loop(tmp_path, session_ttl_minutes=15)
        session = loop.sessions.get_or_create("cli:test")
        _add_turns(session, 6, prefix="hello")
        session.updated_at = datetime.now() - timedelta(minutes=20)
        ...
        # Both _summaries and metadata have the merged recent summary
        ...
        _, summary = loop.auto_compact.prepare_session(reloaded, "cli:test")
        assert summary is not None
        assert summary == session_summary_text(reloaded)
        await loop.close_mcp()

    @pytest.mark.asyncio
    async def test_new_summary_appends_to_recent_summaries(self, tmp_path):
        """A fresh archive appends a new bounded recent summary."""
        loop = _make_loop(tmp_path, session_ttl_minutes=15)
        session = loop.sessions.get_or_create("cli:test")
        _add_turns(session, 6, prefix="hello")
        session.updated_at = datetime.now() - timedelta(minutes=20)
        ...
        async def _fake_archive(messages):
            return _archive_result("First summary.")
        ...
        async def _fake_archive2(messages):
            return _archive_result("Second summary.")
        ...
        _, summary2 = loop.auto_compact.prepare_session(reloaded, "cli:test")
        assert summary2 is not None
        assert "First summary." in summary2
        assert "Second summary." in summary2
        await loop.close_mcp()

    @pytest.mark.asyncio
    async def test_new_command_clears_summary_metadata(self, tmp_path):
        """/new should clear summary metadata so the new session starts fresh."""
        loop = _make_loop(tmp_path, session_ttl_minutes=15)
        session = loop.sessions.get_or_create("cli:test")
        _add_turns(session, 6, prefix="hello")
        session.updated_at = datetime.now() - timedelta(minutes=20)
        ...
        async def _fake_archive(messages):
            return _archive_result("Old summary.")
        ...
        # After /new, metadata should no longer contain summary metadata
        fresh = loop.sessions.get_or_create("cli:test")
        assert "_recent_summaries" not in fresh.metadata

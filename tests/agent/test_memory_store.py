...
class TestMemoryStoreBasicIO:
    ...
    @pytest.mark.parametrize(
        ("method_name", "path_attr"),
        [
            ("write_memory", "memory_file"),
            ("write_soul", "soul_file"),
            ("write_user", "user_file"),
        ],
    )
    def test_memory_state_writes_are_locked_and_atomic(
        self, store, monkeypatch, method_name, path_attr,
    ):
        events = []
        ...
        getattr(store, method_name)("content")

        assert events == ["enter", ("write", getattr(store, path_attr), "content"), "exit"]
...
class TestHistoryWithCursor:
    ...
    def test_compact_history_drops_oldest(self, tmp_path):
        ...
        store.compact_history(processed_through=5)
        entries = store.read_unprocessed_history(since_cursor=0)
        assert len(entries) == 2
...
class TestMemoryRedaction:
    def test_append_history_redacts_sensitive_values_in_raw_jsonl(self, store):
        ...
        text = (
            f"openai={openai_key} github={github_token} auth={bearer} "
            "api_key=abcdefghijk contact=user@example.com "
            "id=110105199001011234 card=1234567890123456 "
            f"{private_key}"
        )

        store.append_history(text)

        history_text = store.history_file.read_text(encoding="utf-8")
        assert openai_key not in history_text
        assert github_token not in history_text
        assert "abcdefghijklmnop" not in history_text
        assert "abcdefghijk" not in history_text
        assert "user@example.com" not in history_text
        assert "110105199001011234" not in history_text
        assert "1234567890123456" not in history_text
        assert "super-secret-key-material" not in history_text
        assert "[REDACTED_SECRET]" in history_text
        assert "[REDACTED_EMAIL]" in history_text
        assert "[REDACTED_ID]" in history_text

    def test_redaction_does_not_restore_raw_when_strip_think_clears_content(self, store):
        store.append_history("<think>api_key=abcdefghijk</think>")

        history_text = store.history_file.read_text(encoding="utf-8")
        entry = json.loads(history_text)
        assert entry["content"] == ""
        assert "abcdefghijk" not in history_text

    def test_secret_assignment_redaction_is_not_greedy(self, store):
        store.append_history("token=abcdefghijk keep-this-visible password: \"supersecret\"")

        history_text = store.history_file.read_text(encoding="utf-8")
        assert "abcdefghijk" not in history_text
        assert "supersecret" not in history_text
        assert "token=[REDACTED_SECRET]" in history_text
        assert 'password: \\"[REDACTED_SECRET]\\"' in history_text
        assert "keep-this-visible" in history_text

    def test_oversized_entry_is_redacted_before_truncation(self, store):
        openai_key = "sk-proj-" + "Z" * 80
        private_key = (
            "-----BEGIN PRIVATE KEY-----\n"
            + "private-key-material-" * 80
            + "\n-----END PRIVATE KEY-----"
        )
        ...
        assert "ZZZZZZ" not in history_text
        assert "BEGIN PRIVATE KEY" not in history_text
        assert "private-key-material" not in history_text
        assert "[REDACTED_SECRET]" in history_text

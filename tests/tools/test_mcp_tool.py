...
@pytest.mark.asyncio
async def test_connect_skips_duplicate_wrapper_without_aborting_server(
    fake_mcp_runtime: dict[str, object | None],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_mcp_runtime["session"] = _make_fake_session_with_capabilities(
    ...
    registry = ToolRegistry()
    registry.register(MCPToolWrapper(SimpleNamespace(call_tool=None), "test", _make_tool_def("tool_a")))
    warnings: list[str] = []
    ...
    assert "mcp_test_tool_a" in registry.tool_names
    assert "mcp_test_tool_b" in registry.tool_names
    assert "mcp_test_resource_res_b" in registry.tool_names
    assert "mcp_test_prompt_prompt_c" in registry.tool_names
    assert any("skipping duplicate capability 'mcp_test_tool_a'" in warning for warning in warnings)
...
def test_sanitize_name_truncates_long_names_with_hash() -> None:
    name = _sanitize_name("mcp_" + ("server_" * 20) + ("tool_" * 20))

    assert len(name) <= 64
    assert name[55] == "_"


def test_sanitize_name_long_names_keep_distinct_hashes() -> None:
    first = _sanitize_name("mcp_" + ("server_" * 20) + "tool_a")
    second = _sanitize_name("mcp_" + ("server_" * 20) + "tool_b")
...
@pytest.mark.asyncio
async def test_connected_mcp_wrapper_requires_runtime_snapshot() -> None:
    tool_def = SimpleNamespace(
        name="demo",
        description="demo tool",
        inputSchema={"type": "object", "properties": {}},
...
@pytest.mark.asyncio
async def test_mcp_wrapper_denies_insufficient_snapshot() -> None:
    tool_def = SimpleNamespace(
        name="demo",
        description="demo tool",
        inputSchema={"type": "object", "properties": {}},
    ...
    wrapper = MCPToolWrapper(SimpleNamespace(call_tool=None), "srv", tool_def)
    wrapper.set_capability_snapshot(
        CapabilitySnapshot(
            version=1,
            source="cron",
            trigger="scheduled",
    ...
    assert exc.value.policy_rule == "capability_mcp_denied"

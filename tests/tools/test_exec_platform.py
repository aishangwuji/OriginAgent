...
class TestSandboxPlatform:
    ...
    async def test_bwrap_skipped_on_windows(self):
        """bwrap must fail closed on Windows instead of running unsandboxed."""
        mock_proc = AsyncMock()
        mock_proc.communicate.return_value = (b"ok", b"")
        mock_proc.returncode = 0
        ...
        assert result == "Error: sandbox 'bwrap' is not supported on Windows; command was not executed"
    ...
    async def test_bwrap_applied_on_unix(self):
        ...
        with (
            ...
            tool = ExecTool(sandbox="bwrap", working_dir="/workspace", restrict_to_workspace=True)
            await tool.execute(command="ls")

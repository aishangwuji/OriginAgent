...
from OriginAgent.agent.tools import (
    ArraySchema,
    IntegerSchema,
    ObjectSchema,
    Schema,
...
def test_validate_params_rejects_unknown_fields_when_additional_properties_false() -> None:
    tool = _SchemaTool(
        {
            "type": "object",
...
def test_validate_params_validates_additional_properties_schema() -> None:
    tool = _SchemaTool(
        {
            "type": "object",
            "properties": {},
    ...
    errors = tool.validate_params({"ok": "yes", "bad": 3})

    assert errors == ["bad should be string"]
...
def test_exec_extract_absolute_paths_captures_home_paths() -> None:
    cmd = "cat ~/.originagent/config.json > ~/out.txt"
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "~/.originagent/config.json" in paths
    assert "~/out.txt" in paths


def test_exec_extract_absolute_paths_captures_quoted_paths() -> None:
    cmd = 'cat "/tmp/data.txt" "~/.originagent/config.json"'
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "/tmp/data.txt" in paths
    assert "~/.originagent/config.json" in paths
...
def test_exec_guard_blocks_home_path_outside_workspace(tmp_path) -> None:
    ...
    error = tool._guard_command("cat ~/.originagent/config.json", str(tmp_path))
...
def test_exec_guard_allows_media_path_outside_workspace(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("OriginAgent.agent.tools.shell.get_media_dir", lambda: media_dir)
...
def test_exec_guard_blocks_windows_drive_root_outside_workspace(monkeypatch) -> None:
    import OriginAgent.agent.tools.shell as shell_mod
...
def test_cast_params_does_not_cast_objects_to_string() -> None:
    tool = CastTestTool(
        {
            "type": "object",
            "properties": {"name": {"type": "string"}},
    ...
    result = tool.cast_params({"name": {"nested": "value"}})

    assert result["name"] == {"nested": "value"}
    errors = tool.validate_params(result)
    assert errors == ["name should be string"]

...
async def test_grep_reports_skipped_binary_and_large_files(
    ...
    tool = GrepTool(
        ...
        limits=ToolLimits(grep_max_file_bytes=10),
    )
...
@pytest.mark.asyncio
async def test_grep_skips_symlink_without_reading_target(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_grep_single_file_symlink_is_skipped(tmp_path: Path) -> None:
    outside = tmp_path.parent / "outside-single-grep.txt"
    outside.write_text("needle\n", encoding="utf-8")
    link = tmp_path / "link.txt"
    try:
        link.symlink_to(outside)
...
@pytest.mark.asyncio
async def test_glob_skips_symlink(tmp_path: Path) -> None:
    outside = tmp_path.parent / "outside-glob.txt"

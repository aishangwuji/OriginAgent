...
class TestReadFileTool:
    ...
    @pytest.mark.asyncio
    async def test_read_rejects_protected_runtime_state(self, tmp_path):
        workspace = tmp_path / "workspace"
        protected = workspace / "memory" / "audit" / "tool_calls.jsonl"
    ...
    @pytest.mark.asyncio
    async def test_read_rejects_oversized_input_by_stat(self, tmp_path):
        f = tmp_path / "large.txt"
        f.write_text("x" * 11, encoding="utf-8")
        tool = ReadFileTool(workspace=tmp_path, limits=ToolLimits(max_input_bytes=10))
...
class TestEditFileTool:
    ...
    @pytest.mark.asyncio
    async def test_edit_rejects_protected_runtime_state(self, tmp_path):
        ...
        result = await tool.execute(
            path="memory/audit/tool_calls.jsonl",
            old_text="old",
            new_text="new",
        )
...
class TestWriteFileTool:
    @pytest.mark.asyncio
    async def test_write_rejects_protected_runtime_state(self, tmp_path):
        tool = WriteFileTool(workspace=tmp_path)

        result = await tool.execute(path="memory/audit/tool_calls.jsonl", content="fake")

        assert "protected runtime state" in result
        assert not (tmp_path / "memory" / "audit" / "tool_calls.jsonl").exists()
    ...
    async def test_write_rejects_media_root_by_default(self, tmp_path, monkeypatch):
        ...
        tool = WriteFileTool(workspace=tmp_path)
        result = await tool.execute(path=str(media / "upload.png"), content="x")

        assert "read-only tool root" in result
...
class TestListDirTool:
    ...
    @pytest.mark.asyncio
    async def test_list_dir_skips_symlink_and_excludes_from_total(self, tool, tmp_path):
        ...
        result = await tool.execute(path=str(tmp_path), max_entries=1)
    ...
    @pytest.mark.asyncio
    async def test_list_dir_recursive_skips_symlink(self, tool, tmp_path):
        outside = tmp_path.parent / "outside-recursive.txt"
        outside.write_text("secret")
        nested = tmp_path / "nested"
        nested.mkdir()
        (nested / "real.txt").write_text("ok")
        ...
        assert "nested/real.txt" in normalized
        assert "link.txt" not in result

...
def test_validate_params_requires_message_only_for_add(tmp_path) -> None:
    ...
    assert any(
        error.startswith("message is required when action='add'")
        for error in tool.validate_params({"action": "add"})
    )
...
def test_validate_params_requires_exactly_one_schedule_for_add(tmp_path) -> None:
    tool = _make_tool(tmp_path)

    no_schedule = tool.validate_params({"action": "add", "message": "hello"})
    multiple = tool.validate_params(
        {
            "action": "add",
            "message": "hello",
            "every_seconds": 60,
    ...
    assert "exactly one of every_seconds, cron_expr, or at is required when action='add'" in no_schedule
    assert "exactly one of every_seconds, cron_expr, or at is required when action='add'" in multiple


def test_validate_params_rejects_short_every_seconds(tmp_path) -> None:
    tool = _make_tool(tmp_path)

    errors = tool.validate_params({"action": "add", "message": "hello", "every_seconds": 59})

    assert "every_seconds must be >= 60" in errors


def test_validate_params_rejects_non_integer_every_seconds(tmp_path) -> None:
    tool = _make_tool(tmp_path)

    string_errors = tool.validate_params({"action": "add", "message": "hello", "every_seconds": "60"})
    bool_errors = tool.validate_params({"action": "add", "message": "hello", "every_seconds": True})

    assert "every_seconds should be integer" in string_errors
    assert "every_seconds must be an integer" in string_errors
    assert "every_seconds should be integer" in bool_errors
    assert "every_seconds must be an integer" in bool_errors


@pytest.mark.asyncio
async def test_execute_rejects_direct_string_every_seconds(tmp_path) -> None:
    tool = _make_tool(tmp_path)
    tool.set_context("telegram", "chat-1")

    result = await tool.execute(action="add", message="hello", every_seconds="60")  # type: ignore[arg-type]

    assert "Error: Invalid parameters for cron:" in result
    assert "every_seconds should be integer" in result
    assert tool._cron.list_jobs() == []


def test_validate_params_rejects_past_or_current_at(tmp_path) -> None:
    tool = _make_tool_with_tz(tmp_path, "UTC")

    errors = tool.validate_params({"action": "add", "message": "hello", "at": "2000-01-01T00:00:00"})

    assert "at must be in the future" in errors


def test_validate_params_rejects_invalid_cron_expr(tmp_path) -> None:
    tool = _make_tool(tmp_path)

    errors = tool.validate_params({"action": "add", "message": "hello", "cron_expr": "not a cron"})

    assert any(error.startswith("invalid cron_expr:") for error in errors)


def test_validate_params_accepts_valid_schedules(tmp_path) -> None:
    tool = _make_tool_with_tz(tmp_path, "UTC")

    assert tool.validate_params({"action": "add", "message": "hello", "every_seconds": 60}) == []
    assert tool.validate_params({"action": "add", "message": "hello", "cron_expr": "0 9 * * *"}) == []
    assert tool.validate_params({"action": "add", "message": "hello", "at": "2999-01-01T00:00:00"}) == []

...
## Governed Self-Tuning Config Overlay

v2.2 adds governed self-tuning for evolution config. It does not edit the main config file. Automatic tuning writes only to:

```text
memory/evolution_config_overrides.json
memory/evolution_config_patches.jsonl
```

The overlay is applied at runtime when evolution services read `learning.evolution`. The patch log records each applied or rejected patch with actor, source, path, old value, new value, and evidence.

Self-tuning may only tighten safety:

- set `dry_run=true`.
- move `mode` toward `conservative`.
- raise `workflow_priority_threshold`.
- reduce `max_proposals_per_cycle`.
- disable `skill_candidates_enabled`.
- disable `auto_verify_workflows`.
- require trial isolation and read-only trial tools.
- require trial blocklists for side-effecting tools.

Self-tuning may not:

- set `allow_manual_override=true`.
- set `dry_run=false`.
- move `mode` toward `exploratory` or `aggressive`.
- enable write, exec, message, cron, or spawn during trial.
- modify provider keys, workspace paths, tool permissions, system prompts, or unrelated config.

`originagent_runtime_status.evolution.config_overlay` reports the active overlay, recent patches, and store paths. The control plane exposes:

- `list_config_overlay`: read-only overlay inspection.
- `clear_config_overlay`: guarded write that clears overrides and requires `learning.evolution.allow_manual_override=true`.

Clearing the overlay is an operator action. It is not automatic.
...
- The main config file is not modified automatically.
- Automatic config self-tuning can only write safety-tightening overlay patches.
...
- `config_overlay`: governed self-tuning overlay status and recent patches.

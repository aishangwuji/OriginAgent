...
1. 后台认知闭环已进入代码、测试和 introspection。
2. 当前工作重点是收口、验收和差距显性化，而不是继续按“待设计”或“待实现”口径描述。
3. `RQ-001`、`RQ-005`、`P1 Integration` 应统一改写为 `Implemented, Closeout Validation Pending`。
4. `Phase 1` 结束前仍需保留少量显式差距条目，但这些差距不等于主线尚未落地。
...
| `scheduler_runs.jsonl` 只在 scheduler 扫描路径产出 | `CognitiveScheduler.run_once()` 末尾写入 `scheduler_runs.jsonl`；fallback `CognitiveLoop` 路径直接调用 session processor，不写该文件 | 已有代码无测试证据 | 文档修正：显式写出主路径与 fallback 路径审计产物差异 |
...
| cooldown 判断逻辑来自 `ActiveIntentService` | 认知运行时直接调用 `_passes_cooldown()` 复用 session / intent cooldown 规则 | 已有测试 | Phase 1 接受的技术债：保留私有方法跨类调用并登记为近端卫生候选 |
...
| 双轨审计在当前实现中并存 | 同一次决策会同时写认知审计 JSONL 与 `memory/active_intents/records.jsonl` | 待补验证 | Phase 1 接受的技术债：本轮冻结现状，不合并模型 |
...
`Phase 1` 收口验证采用“现有测试优先、缺口最小补齐”的策略。当前优先验证四组证据：

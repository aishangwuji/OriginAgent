# OpenHome v1 开发任务清单与技术落地方案

> 状态：历史迁移参考。项目对外身份正在从 OpenHome 迁移为 OriginAgent；智能家居能力后续应作为领域插件保留，而不是核心 Agent 架构身份。

> 目标：把当前代码库直接产品化为 OpenHome。
> OpenHome 不是一个可选模式，而是这个项目最终对用户呈现的唯一产品身份。
...
- 项目默认名称、命令、配置目录、workspace、模板、Docker 服务名统一为 OpenHome。
- OpenHome 默认定位为“运行在本地 NAS 上的 AI 智能家居管家”。
- Telegram 作为第一入口可用。
...
OpenHome gateway
  ├─ ChannelManager
  ├─ AgentLoop
  ├─ OpenHome workspace profile
  ├─ Home Assistant MCP tools
  ├─ Provider fallback
  ├─ Dream / memory
  └─ OpenHome event ingress
        ↑
        │
OpenHome trigger
...
- OpenHome 直接复用现有 agent loop、message bus、channel、MCP、memory、provider、Docker 能力。
- 不设计 `openhome.enabled` 之类的模式开关。
- 默认配置、默认模板、默认命令都以 OpenHome 为中心。
- v1 主入口是 Telegram；现有 webui 不作为 OpenHome 的核心产品入口。
...
目标：运行、安装、阅读、日志、回复里看到的都是 OpenHome。

任务：

- 项目包名改为 `openhome-ai` 或 `openhome`。
- CLI 命令统一为 `openhome`。
- 默认 `botName` 改为 `OpenHome`。
- 默认 `botIcon` 改为适合智能家居的符号。
- CLI help、gateway 启动文案、status 文案统一改为 OpenHome。
- README 改成 OpenHome 产品说明。
- Docker 镜像、服务名、容器名统一为 OpenHome。

验收标准：

- `openhome --help` 展示 OpenHome。
- `openhome gateway` 启动日志展示 OpenHome。
...
- 默认 config path 改为 `~/.openhome/config.json`。
- 默认 workspace 改为 `~/.openhome/workspace`。
...
- 新用户第一次初始化生成 `~/.openhome/config.json`。
- 配置能解析 HA URL、HA Token、Telegram Token。
- `openhome status` 能显示 HA、Telegram、模型、workspace 状态。

---

### C. OpenHome workspace profile

目标：系统提示词默认就是家庭管家。

任务：

- 重写 `SOUL.md`：OpenHome 的身份、安全边界、主动但克制的通知风格。
...
目标：让 OpenHome 通过安全工具访问 HA，而不是让模型拼 HTTP 请求。
...
- 服务调用写入 `~/.openhome/behavior.jsonl`。
...
```json
{
  "tools": {
    "mcpServers": {
      "home_assistant": {
        "command": "python",
        "args": ["-m", "openhome.openhome.ha_mcp"],
        "env": {
          "HA_URL": "${HA_URL}",
          "HA_TOKEN": "${HA_TOKEN}"
        },
...
- `allowFrom` 不允许默认开放。
- `openhome status` 显示 Telegram allowlist 状态。
- 提供 OpenHome 示例命令。
...
- `notify`：直接发送通知。
- `model_judge`：交给 OpenHome 判断是否需要通知或执行动作。
...
services:
  openhome-gateway:
    build: ./openhome
    command: ["gateway", "--config", "/home/openhome/.openhome/config.json"]
    volumes:
      - ./data/openhome:/home/openhome/.openhome
    ports:
      - "18790:18790"
      - "8765:8765"
  ...
  openhome-trigger:
    build: ./openhome
    command: ["trigger", "--config", "/home/openhome/.openhome/config.json"]
    volumes:
      - ./data/openhome:/home/openhome/.openhome
```
...
- 产品名、CLI、默认路径、模板、README、Docker 服务名统一为 OpenHome。
- 增加 `homeAssistant` 配置。
- 实现 HA REST client。
- 实现 HA MCP 最小工具。
- Telegram 手工完成查询和控制。

---
name: update-setup
description: One-time setup wizard for the OpenHome upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `OpenHome --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v OpenHome || true
python -m pip show OpenHome || true
pipx list | sed -n '/OpenHome/,+3p' || true
uv tool list | sed -n '/OpenHome/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how OpenHome was
installed.
...
`question: "Where is your OpenHome source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "OpenHome[EXTRAS]" --force` |
| pipx | `pipx install --force "OpenHome[EXTRAS]"` |
| pip | `python -m pip install --upgrade "OpenHome[EXTRAS]"` |
...
---
name: update
description: "Upgrade OpenHome to the latest version. Triggers: upgrade OpenHome, update OpenHome, 升级OpenHome, 更新OpenHome."
---
...
4. Use `exec` to verify: `OpenHome --version`
5. Tell the user the new version. Say: "Run `/restart` to restart OpenHome and apply the update. If `/restart` is unavailable in this channel, restart the OpenHome process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade OpenHome' when you want to update."

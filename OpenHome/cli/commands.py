...
# Remove default handler and re-add with unified OpenHome format
logger.remove()
...
from OpenHome.config.paths import get_workspace_path, is_default_workspace
...
app = typer.Typer(
    ...
    help=f"{__logo__} OpenHome - Local AI Home Assistant",
...
def _init_prompt_session() -> None:
    ...
    from OpenHome.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} OpenHome[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} OpenHome[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} OpenHome v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """OpenHome - Local AI Home Assistant."""
    pass
...
def onboard(
    ...
    """Initialize OpenHome configuration and workspace."""
    from OpenHome.config.loader import get_config_path, load_config, save_config, set_config_path
    from OpenHome.config.schema import Config
    ...
    if wizard:
        from OpenHome.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'openhome onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'openhome agent -m "Hello!"'
    gateway_cmd = "openhome gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} OpenHome is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See the OpenHome channel docs for setup.[/dim]"
    )
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from OpenHome.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from OpenHome.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show OpenHome runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'OpenHome[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from OpenHome.api.server import create_app
    from OpenHome.bus.queue import MessageBus
    from OpenHome.session.manager import SessionManager

    if verbose:
        logger.enable("OpenHome")
    else:
        logger.disable("OpenHome")
...
def _run_gateway(
    ...
    *,
    ...
    from OpenHome.cron.types import CronJob
    from OpenHome.heartbeat.service import HeartbeatService
    from OpenHome.providers.factory import build_provider_snapshot, load_provider_snapshot
    from OpenHome.session.manager import SessionManager

    port = port if port is not None else config.gateway.port

    console.print(f"{__logo__} Starting OpenHome gateway version {__version__} on port {port}...")
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show OpenHome runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from OpenHome.channels.registry import discover_all
    from OpenHome.config.loader import load_config, set_config_path
...
def _get_bridge_dir() -> Path:
    ...
    from OpenHome.config.paths import get_bridge_install_dir

    user_bridge = get_bridge_install_dir()
    stamp_file = user_bridge / ".openhome-bridge-source-hash"

    # Find source bridge: first check package data, then source dir
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # OpenHome/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall OpenHome")
        raise typer.Exit(1)
...
@app.command()
def status():
    """Show OpenHome status."""
    from OpenHome.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} OpenHome Status\n")
...
def _login_github_copilot() -> None:
    try:
        from OpenHome.providers.github_copilot_provider import login_github_copilot

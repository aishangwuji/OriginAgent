...
from OpenHome.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/OpenHome",
    "X-OpenRouter-Title": "OpenHome",
...
def _openai_compat_timeout_s() -> float:
    ...
    return _float_env("OPENHOME_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S)
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply OpenHome attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("OPENHOME_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(

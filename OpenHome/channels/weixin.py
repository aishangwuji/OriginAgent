...
from OpenHome.config.paths import get_media_dir, get_runtime_subdir
from OpenHome.config.schema import Base
from OpenHome.utils.helpers import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.openhome/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                self.logger.error("login failed. Run 'OpenHome channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"OpenHome-{uuid.uuid4().hex[:12]}"

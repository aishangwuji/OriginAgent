...
class SkillLifecycleStore:
    ...
    def transition(
        ...
        *,
        ...
        with self._locked():
            ...
            metadata = _metadata_openhome(frontmatter)
            ...
            frontmatter["metadata"] = _set_metadata_openhome(frontmatter.get("metadata"), metadata)
    ...
    def decorate_entry(
        ...
        *,
        ...
        openhome = _metadata_openhome(frontmatter)
        lifecycle, verification = _derive_status(source, openhome)
        ...
        always = bool(frontmatter.get("always") or openhome.get("always"))
        ...
        record = {
            **entry,
            ...
            "proposal_status": _clean(openhome.get("proposal_status") or "unknown", 128),
            "verification_status": verification,
            "lifecycle_status": lifecycle,
            "version": _clean(openhome.get("version") or "1", 128),
            "supersedes_skill": _clean(openhome.get("supersedes_skill") or "", 128),
            "reviewed_by": _clean(openhome.get("reviewed_by") or "", 128),
            "reviewed_at": _clean(openhome.get("reviewed_at") or "", 128),
            "review_proposal_id": _clean(openhome.get("review_proposal_id") or "", 256),
            "domain_id": _clean(openhome.get("domain_id") or "core", 128),
            "created_by": _clean(openhome.get("created_by") or "", 128),
            "last_lifecycle_event_id": _clean(openhome.get("last_lifecycle_event_id") or "", 128),
            "always": always,
...
def _metadata_openhome(frontmatter: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(raw, dict):
        ...
        return {}
    value = data.get("OpenHome", data.get("openclaw", {}))
    return dict(value) if isinstance(value, dict) else {}


def _set_metadata_openhome(raw: Any, openhome: dict[str, Any]) -> dict[str, Any]:

...
from OpenHome.agent.facts import FactStore, summarize_facts
...
class SelfModelRenderer:
    ...
    def render(self, self_model: dict[str, Any]) -> str:
        ...
        lines = [
            "# Self Model",
            "",
            f"- Agent: `{identity.get('agent_name') or 'OpenHome'}`",
...
def _is_template_content(content: str, template_path: str) -> bool:
    with suppress(Exception):
        template = pkg_files("OpenHome") / "templates" / template_path
        if template.is_file():

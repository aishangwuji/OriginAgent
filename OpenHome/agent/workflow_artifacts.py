...
def validate_workflow_artifact_content(
    ...
    *,
    ...
    openhome_meta = metadata.get("OpenHome")
    if not isinstance(openhome_meta, dict):
        raise ValueError("metadata.OpenHome is required")
    required = {
        "proposal_status": "proposed",
        "verification_status": "unverified",
        "created_by": "background_review",
    ...
    for key, expected in required.items():
        if openhome_meta.get(key) != expected:
            raise ValueError(f"metadata.OpenHome.{key} must be {expected}")
    if expected_proposal_id and openhome_meta.get("review_proposal_id") != expected_proposal_id:
        raise ValueError("metadata.OpenHome.review_proposal_id is incorrect")
    if expected_domain_id and openhome_meta.get("domain_id") != expected_domain_id:
        raise ValueError("metadata.OpenHome.domain_id is incorrect")

...
export function useSessions(): {
  ...
  const { client, token, refreshToken } = useClient();
  ...
  const deleteChat = useCallback(
    async (key: string) => {
      await withTokenRefresh(tokenRef.current, refreshTokenRef.current, (freshToken) =>
        apiDeleteSession(freshToken, key),
      );
...
export function useSessionHistory(key: string | null): {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const body = await withTokenRefresh(token, refreshToken, (freshToken) =>
          fetchWebuiThread(freshToken, key),
        );
        if (cancelled) return;
  ...
  }, [key, refreshSeq, refreshToken, token]);
...
export function sessionTitle(
  ...
  return deriveTitle(
    ...
    i18n.t("chat.pendingTitle", { defaultValue: i18n.t("chat.newChat") }),
  );
}

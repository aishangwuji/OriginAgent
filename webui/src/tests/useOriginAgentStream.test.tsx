...
describe("useOriginAgentStream", () => {
  it("starts in streaming mode when history shows pending tool calls", () => {
    ...
    const { result } = renderHook(
      () => useOriginAgentStream("chat-p", initialMessages, true),
      {
  ...
  it("collapses consecutive tool_hint frames into one trace row", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-t", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("treats progress with arbitrary agent_ui like ordinary trace text", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-au", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
    act(() => {
      fake.emit("chat-au", {
  ...
  it("renders live tool traces from structured tool events", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-tool-events", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps finish tool events for activity counters without duplicating traces", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-tool-finish", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("accumulates reasoning_delta chunks on a placeholder until reasoning_end", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-r", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("absorbs a streaming reasoning placeholder into the answer turn that follows", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-r2", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("ignores empty reasoning_delta frames", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-r3", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("treats legacy kind=reasoning messages as a complete delta + end pair", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-r4", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("attaches post-hoc reasoning to the same assistant turn above the answer", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-r5", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("does not attach a new turn's reasoning across the latest user boundary", () => {
    ...
    const { result } = renderHook(
      () => useOriginAgentStream("chat-r6", initialMessages),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("does not attach reasoning across a tool trace boundary", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-r7", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps tool-call reasoning before the matching live tool trace", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-tool-reasoning", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("absorbs non-streamed final answers into the preceding reasoning placeholder", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-final-reasoning", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("prunes reasoning-only placeholders when a turn ends without an answer", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-empty-thinking", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("drops stale reasoning-only placeholders before sending the next user turn", () => {
    ...
    const { result } = renderHook(
      () => useOriginAgentStream("chat-stale-thinking", initialMessages),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("attaches assistant media_urls to complete messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-m", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("suppresses redundant stream confirmation after assistant media", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-img-result", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("passes image generation options to the websocket client", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-img", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("stops the active turn without adding a user slash command bubble", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-stop", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps streaming alive across stream_end and completes on turn_end", () => {
    ...
    const { result } = renderHook(() => useOriginAgentStream("chat-s", EMPTY_MESSAGES, false, onTurnEnd), {
      wrapper: wrap(fake.client),
    });
  ...
  it("stamps latency on the last assistant bubble from turn_end", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-lat", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("tracks goal_status running and clears on idle", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useOriginAgentStream("chat-g", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("restores runStartedAt after switching away and back when goal_status was recorded without a subscriber", () => {
    ...
    const { result, rerender } = renderHook(
      ({ chatId }: { chatId: string }) => useOriginAgentStream(chatId, EMPTY_MESSAGES),
      {
        wrapper: wrap(fake.client),

...
describe("MessageBubble", () => {
  it("does not expose raw markdown while the renderer chunk is loading", () => {
    ...
    expect(screen.queryByText(/### Raw heading/)).not.toBeInTheDocument();
  ...
  it("wraps long unbroken user text inside the message bounds", () => {
    const token =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJkMjY1ZTVkNTU0ZGQ0M2FlOTRiNDRmODViM2FmZDAzMCIsImlhdCI6MTc3OTA4NzgzNywiZXhwIjoyMDk0NDQ3ODM3fQ.poMnIiePP_uLgW9G69KEFTqkAZ3Omlg5F4w9pyJ0G6k";
    const message: UIMessage = {
      id: "u-long",
      role: "user",
      content: token,
    ...
    expect(row).toHaveClass("min-w-0", "max-w-[min(85%,36rem)]");
    expect(pill).toHaveClass("max-w-full", "[overflow-wrap:anywhere]");
  });

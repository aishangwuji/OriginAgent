...
import { listSignals, updateSignal, fetchEvolutionStatus, userFriendlyError } from "@/lib/api";
        ...
        {loading ? (
          ...
          <div className="rounded-lg border border-dashed border-border/70 bg-background/45 p-8 text-center text-sm text-muted-foreground">
            ...
            <p className="mt-2 text-xs text-muted-foreground/70">
              {t("signals.emptyHint", "Signals are generated by the evolution engine from detected usage patterns. Try changing the status or kind filter above.")}
            </p>
          </div>
        ) : (

...
export function ThreadShell({
  ...
  const { client, modelName, token, refreshToken } = useClient();
  ...
  const showHeroComposer = messages.length === 0 && !loading && !hasActiveSession;
  ...
  useEffect(() => {
    if (loading || historical.length === 0) return;
    if (
      historical.some(
        (message) =>
          message.role === "assistant" &&
          (message.content.trim().length > 0 || (message.reasoning?.trim().length ?? 0) > 0),
      )
    ) {
      preloadMarkdownText();
    }
  ...
  const composer = (
    <>
      {hasActiveSession && historyError && messages.length > 0 ? (
        <div className="mb-3 rounded-[16px] border border-destructive/20 bg-destructive/5 px-4 py-3 text-sm text-destructive">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="font-medium">
                {t("thread.loadErrorTitle", { defaultValue: "Could not load this conversation" })}
              </p>
              <p className="mt-1 text-xs opacity-80">{historyError}</p>
            </div>
            <button
              type="button"
              onClick={refreshHistory}
              className="self-start rounded-full border border-destructive/25 px-3 py-1 text-xs font-medium transition-colors hover:bg-destructive/10 sm:self-center"
            >
              {t("thread.retryLoad", { defaultValue: "Retry" })}
            </button>
          </div>
        </div>
      ) : null}
  ...
  return (
    <section className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <ThreadViewport
        ...
        pendingThreadLayout={loading && hasActiveSession}
      />
    </section>
  );
}

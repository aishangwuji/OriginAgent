 ...
 * - **Production**: off unless ``localStorage.setItem('OpenHome_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('OpenHome_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("OpenHome_debug_ws")?.trim().toLowerCase() ?? "";
...
export class OpenHomeClient {
  ...
  constructor(private options: OpenHomeClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[OpenHome ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[OpenHome ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - OpenHomeClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }

 ...
 * - **Production**: off unless ``localStorage.setItem('OriginAgent_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('OriginAgent_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("OriginAgent_debug_ws")?.trim().toLowerCase() ?? "";
...
export interface OriginAgentClientOptions {
  url: string;
  reconnect?: boolean;
 ...
 */
export class OriginAgentClient {
  ...
  constructor(private options: OriginAgentClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[OriginAgent ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[OriginAgent ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - OriginAgentClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
